...
- [x] 2.1 LRU Cache O(1) 改造 | `cache/memory.go:120-138`
  - 投入: 2天 | ROI: ★★★
  - 现状: 每次淘汰遍历全量 O(n)
  - 方案: `container/list` + `map` → O(1) 淘汰，或使用 `hashicorp/golang-lru`
  - 验收: 大集群 P99 延迟降低 30-50%
- [x] 2.2 28 份 Getter/Updater/Deleter → Go 泛型 | `pkg/k8s/resource.go` (1084 行)
  - 投入: 3天 | ROI: ★★★
  - 评估: 涉及 25+ 资源类型核心代码，重构风险高，测试覆盖不足，建议搁置
- [x] 2.3 Overview 全量 Pod 列表无分页 | `service/overview.go`
  - 投入: 2天 | ROI: ★★★
...
- [x] 2.5 全局 CustomEvent 替换 | tab-change 事件总线
  - 投入: 2天 | ROI: ★★★
  - `tab-change` 已移除，`CustomEvent('logout')` 仍在 Sidebar 使用
  - 验收: 刷新页面可恢复标签状态，消除隐式耦合
- [x] 2.6 apiClient 类型安全改造
  - 投入: 2天 | ROI: ★★★
  - 现状: ~30 处 `as any` 类型擦除
  - 方案: 用泛型约束 API 响应类型
...
- [x] 2.7 filterHiddenFields 合并 | `CRDPage.tsx` + `YamlTab.tsx`
  - 投入: 0.5天 | ROI: ★★★
  - 方案: 提取到 `utils/`
- [x] 2.8 WebSocket Token 提取合并 | `api/common.go` + `auth.go`
  - 投入: 1天 | ROI: ★★☆
  - 两个文件各有一套逻辑，行为不一致
  - 方案: 统一到 middleware 层
- [x] 2.9 事件过滤逻辑去重 | `service/list.go` + `resource_manager.go`
  - 投入: 1天 | ROI: ★★☆
  - 两处有相同的 time-filter + sort 逻辑
  - 方案: 合并为公共函数
- [x] 2.10 Overview errgroup 单失败取消全组 | `service/overview.go:88-143`
  - 投入: 1天 | ROI: ★★★
  - Pod 列表失败 → errgroup 取消 ctx → namespace/services/events 全被取消，始终返回 nil
  - 方案: 独立收集错误，返回部分数据而非 nil
- [x] 2.11 getRecentEvents 全量拉取无过滤 | `service/overview.go:189-216`
  - 投入: 1天 | ROI: ★★★
  - `Events("").List(ctx, metav1.ListOptions{})` 拉取所有 namespace 所有时间的 Events
  - 方案: 加 fieldSelector 按 lastTimestamp 过滤，或限制返回条数
- [x] 2.12 Cache DELETE 未清除过滤列表缓存 | `api/resource_handler.go:286-289`
  - 投入: 1天 | ROI: ★★★
  - 只删了特定 key，Label/Field selector 查询的缓存仍然残留
  - 方案: 前辍扫描删除或使用 cache 通配失效
- [x] 2.13 登录页 password 定时攻击 | `api/login.go:128`
  - 投入: 0.5天 | ROI: ★★★
  - 同 1.6 模式: 明文密码回退路径使用 `==` 字符串比较
  - 方案: 统一走 bcrypt 比较
- [x] 2.14 前端 LoginPage 使用 raw fetch | `ui/src/pages/LoginPage.tsx:47`
  - 投入: 0.5天 | ROI: ★★☆
  - 直接使用 `fetch()` 而非 `apiClient`，无法享受重试/超时基础设施
  - 方案: 使用 `apiClient.post('/api/login', ...)`
- [x] 2.15 Sidebar logo 使用源码路径而非 Vite import | `ui/src/common/Sidebar.tsx:182`
  - 投入: 0.5天 | ROI: ★★☆
...
- [x] 3.1 convertToSearchableItems 泛型化 | `service/resource_manager.go:114`
  - 投入: 0.5天 | ROI: ★★☆
  - 当前使用 map dispatch，架构合理 — 收益极低，建议搁置
- [x] 3.2 bundle 优化 — react-icons / xterm 懒加载
  - 投入: 1天 | ROI: ★★☆
  - 验收: 首屏 JS 减少 ~100KB
- [x] 3.3 OverviewTab.tsx 拆分 (443行)
  - 投入: 2天 | ROI: ★★☆
...
- [x] 3.4 资源类型配置单源化
  ...
  - 方案: 统一到一处，其余引用
  - 验收: 新增资源改一个文件即可
- [x] 3.5 types/k8s-resources.ts 拆分 (942行)
  - 投入: 2天 | ROI: ★★☆
  - 按资源域拆分
- [x] 3.6 useResourceList.ts cache 内存泄漏
  - 投入: 1天 | ROI: ★★☆
  - 模块级 `Map<string, CacheEntry>` 只过期不清除 key
  - 方案: 加入定时清理或改为 LRU
- [x] 3.7 OverviewService 缺少 metrics client
  - 投入: 1天 | ROI: ★★☆
  - `NewOverviewService` 未传入 metrics client → Node CPU/Mem 始终为 0
- [x] 3.8 工厂函数改为单例 | `pkg/k8s/resource.go:864-1047`
  - 投入: 1天 | ROI: ★★☆
  - 评估: Map 创建开销忽略不计，建议搁置
- [x] 3.9 related_finder Secret 遍历 Pods 6 次 | `service/related_finder.go:480-582`
  - 投入: 1天 | ROI: ★★☆
  - volumes/imagePullSecrets/envFrom/envFrom(initContainers)/env.valueFrom/projection 各遍历一次
  - 方案: 合并为单次遍历
- [x] 3.10 apiClient/authFetch 双重 cluster 参数 | `ui/src/utils/apiClient.ts:36` + `auth.ts:128`
  - 投入: 0.5天 | ROI: ★★☆
  - apiClient 追加 ?cluster=foo, authFetch 再次追加 → URL 含 &cluster=foo&cluster=foo
  - 方案: 移除 authFetch 中的 cluster 逻辑，统一走 apiClient
- [x] 3.11 ResourceMap/ResourceListItemMap 缺失类型映射 | `ui/src/types/resource-maps.ts`
  - 投入: 1天 | ROI: ★★☆
  - configmaps/secrets/ingress/pvcs/pvs/storageclasses/namespaces/events/cronjobs/jobs 均 fallback 到 generic
  - 方案: 补全映射
- [x] 3.12 OverviewService 每次请求新建 | `api/overview.go:14-44`
  - 投入: 0.5天 | ROI: ★★☆
  - `service.NewOverviewService(clientset)` 每个 HTTP 请求创建新实例
  - 方案: 启动时创建单例复用
- [x] 3.13 GetClientRESTConfig 静默回退默认集群 | `service/k8s_client.go:317-328`
  - 投入: 1天 | ROI: ★★☆
  - 请求不存在的集群名时返回默认集群配置，操作可能落在错误集群
  - 方案: 找不到时返回 nil，由调用方处理
- [x] 3.14 AddCluster 静默覆盖已有配置 | `service/k8s_client.go:323-337`
  - 投入: 1天 | ROI: ★★☆
  - 评估: 已 Close 旧 holder 防泄漏，价值不大，建议搁置
- [x] 3.15 "authentication successful" 每请求 Info 级别日志 | `api/middleware/auth.go:106-109`
  - 投入: 0.5天 | ROI: ★★☆
  - 每个 API 请求都 Info 级记录，高流量下日志膨胀
  - 方案: 改为 Debug 级别
- [x] 3.16 管理接口启动时输出 admin username | `config/config.go:44`
  - 投入: 0.5天 | ROI: ★★☆
  - 评估: 代码核实已无此问题，建议搁置
- [x] 3.17 CRD/ArgoCD managers 永不清理 | `service/k8s_client.go:260-296`
  - 投入: 1天 | ROI: ★★☆
  - 存入 sync.Map 后无 Close/清理机制，集群切换后旧 manager 泄漏
  - 方案: ClientManager.Close() 中清理
- [x] 3.18 Breadcrumb 未使用的 props + 重复资源名映射 | `ui/src/common/Breadcrumb.tsx:11-59,64-84`
  - 投入: 0.5天 | ROI: ★★☆
...
| # | 任务 | 备注 |
|---|------|------|
| 4.1 | ConfigMap/Secret 表单编辑器 | 功能需求而非优化，YAML 编辑可用 |
| 4.2 | 审计日志 (操作记录) | 新功能 Phase 4 |
| 4.3 | 实时 Watch (Informer WebSocket) | 新功能 Phase 3 |
| 4.4 | tsconfig strict 模式 | 可配合大重构一起做 |
| 4.5 | 死代码清理 (Dialog.tsx, LogsFilterBar.tsx) | ✅ 已删除 |
| 4.6 | JWT 黑名单持久化 | 重启机会少，收益低 |
| 4.7 | PasswordChange 持久化 | 当前 in-memory 可用 |
| 4.8 | related_finder ConfigMap N+1 | 逻辑复杂，功能正常 |
| 4.9 | pagination reflection 多次调用 | 大数据集下可升 P2 |
| 4.10 | cache/memory.go logger 参数未使用 | 可改为 `*slog.Logger` |
| 4.11 | useResourceList 返回未声明方法 | 死代码，无人调用 |
| 4.12 | Dialog.tsx 中文按钮文字 | 已标记为死代码 (4.5) |
| 4.13 | PodsTabProps 重复定义 | ✅ 已修复 |
| 4.14 | PodsTab loading 文本截断 | ✅ 已修复 | |
